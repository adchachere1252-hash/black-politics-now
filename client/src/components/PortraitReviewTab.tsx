...
import { CheckCircle2, ExternalLink, ImagePlus, Search, ShieldCheck, XCircle } from "lucide-react";
...
export function PortraitReviewTab({ initialTargetKey }: { initialTargetKey?: string }) {
  ...
  const { data: managementTargets = [], refetch: refetchManagementTargets } = trpc.portraits.managementTargets.useQuery();
  ...
  const [researchPackage, setResearchPackage] = useState<{ task: { id: number; status: string; executionError?: string | null; agentWorkPackageSources?: string | null }; proposals: Array<{ title?: string; proposedValue?: string; evidence?: string | null }> } | null>(null);
  const [bulkIndex, setBulkIndex] = useState(0);
  const [brokenBulkImages, setBrokenBulkImages] = useState<number[]>([]);
  const [showAllCandidates, setShowAllCandidates] = useState(false);
  const [selectedPreviewUnavailable, setSelectedPreviewUnavailable] = useState(false);
  const managedTargetList = (showAllCandidates ? managementTargets : targets) as Array<Target & { currentPhotoUrl?: string | null }>;
  const targetByKey = useMemo(() => new Map(managedTargetList.map((target) => [keyFor(target), target])), [managedTargetList]);
  const visibleTargets = useMemo(() => {
    const query = candidateQuery.trim().toLowerCase();
    if (!query) return managedTargetList;
    return managedTargetList.filter((target) => `${target.candidateName} ${target.location} ${target.targetType}`.toLowerCase().includes(query));
  }, [candidateQuery, managedTargetList]);
  ...
  useEffect(() => {
    setSelectedPreviewUnavailable(false);
  }, [selectedPending?.id]);

  const refresh = () => {
    void refetchTargets();
    void refetchManagementTargets();
    void refetchPending();
  ...
  const submit = trpc.portraits.submit.useMutation({
    onSuccess: (saved) => {
      ...
      setNote("");
      setFormError(null);
      setReceipt(`Saved. ${saved.candidateName}'s source-backed portrait is now in the Approve / Deny queue.`);
      refresh();
    },
    onError: (error) => setFormError(error.message),
  });
  const review = trpc.portraits.review.useMutation({
    onSuccess: (saved) => {
      setReceipt(saved.status === "approved" ? `Approved. ${saved.candidateName}'s public portrait and private review record refreshed.` : `Denied. ${saved.candidateName}'s reason was saved and the public portrait was left unchanged.`);
      setFormError(null);
      refresh();
    },
    onError: (error) => setFormError(error.message),
  });
  const upload = trpc.portraits.upload.useMutation({
    onSuccess: ({ url }) => { setImageUrl(url); setFormError(null); setReceipt("Image file saved to secure storage. Add the provenance source, then send it to Approve / Deny."); },
    onError: (error) => setFormError(error.message),
  });
  const researchNow = trpc.portraits.researchNow.useMutation({
    onSuccess: (result) => {
      setResearchPackage(result as typeof researchPackage);
      const evidence = getPortraitApprovalEvidence(result.proposals ?? [], parseSources(result.task?.agentWorkPackageSources));
      if (evidence) {
        setImageUrl(evidence.imageUrl);
        setSourceUrl(evidence.sourceUrl);
        setNote(`${evidence.proposalTitle} — ${evidence.sourceTitle}`);
        setFormError(null);
        setReceipt("Research found a reviewable source package. Confirm the image and provenance below, then send it to Approve / Deny. Nothing has been published.");
        return;
      }
      setReceipt(null);
      setFormError(result.task?.executionError || "Research finished without a reviewable direct image package. Open an official-source lead below, then submit a direct image URL and provenance page for review.");
    },
    onError: (error) => setFormError(error.message),
  });
  ...
  const uploadFile = (file?: File) => {
    ...
    if (!accepted.includes(file.type)) return setFormError("Upload a JPG, PNG, WEBP, or GIF portrait.");
    if (file.size > 5 * 1024 * 1024) return setFormError("Portrait image must be 5 MB or smaller.");
    const reader = new FileReader();
    reader.onload = () => upload.mutate({ fileName: file.name, contentType: file.type as "image/jpeg" | "image/png" | "image/webp" | "image/gif", dataBase64: String(reader.result || "") });
    reader.onerror = () => setFormError("The selected image could not be read for upload.");
    reader.readAsDataURL(file);
  };

  const requestPortraitResearch = () => {
    if (!selectedTarget) return setFormError("Choose a candidate before requesting portrait research.");
    setFormError(null);
    setReceipt(null);
    setResearchPackage(null);
    researchNow.mutate({
      targetType: selectedTarget.targetType,
      targetRecordId: selectedTarget.targetRecordId,
      targetPhotoField: selectedTarget.targetPhotoField,
      candidateName: selectedTarget.candidateName,
      sourceLead: sourceLeads[0]?.url,
    });
  };

  const sendEvidenceForReview = () => {
    if (!selectedTarget) return setFormError("Choose a candidate before submitting image evidence.");
    const managedPortrait = imageUrl.trim().startsWith("/manus-storage/candidate-portraits/");
    if ((!/^https:\/\//.test(imageUrl.trim()) && !managedPortrait) || !/^https:\/\//.test(sourceUrl.trim())) return setFormError("Add a secure external image URL or uploaded portrait plus a secure HTTPS source link before sending this image to review.");
    setFormError(null);
    setReceipt(null);
  ...
  const decide = (decision: "approved" | "rejected") => {
    ...
    if (decision === "approved" && selectedPreviewUnavailable) return setFormError("This proposed image did not load. It cannot be approved; deny it with a note or replace the source package.");
  ...
  return <div className="space-y-5">
    <section className="glass-card rounded-xl p-5">
      ...
      <div className="mt-4 rounded-lg border border-primary/25 bg-primary/[0.035] p-3 text-xs leading-5 text-muted-foreground"><strong className="text-foreground">Four clear steps:</strong> <strong>1.</strong> Find and choose a candidate. <strong>2.</strong> Check available evidence; use a returned package or an official-source lead. <strong>3.</strong> Add a direct image and provenance page to the review queue. <strong>4.</strong> Approve or deny the image. Nothing becomes public before your approval.</div>
    </section>

    <section className="glass-card rounded-xl p-5">
      <div className="flex flex-wrap items-end justify-between gap-3"><div className="grid min-w-0 flex-1 gap-3 md:grid-cols-[minmax(200px,.75fr)_minmax(260px,1.25fr)]"><label className="block text-sm font-bold">Find a candidate<span className="relative mt-2 block"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"/><input value={candidateQuery} onChange={(event) => { setCandidateQuery(event.target.value); setFormError(null); }} placeholder="Search name, state, district, or office" className="w-full rounded-md border border-border bg-background py-2.5 pl-8 pr-3 text-sm font-normal"/></span><span className="mt-1 block text-[11px] font-normal text-muted-foreground">{visibleTargets.length} matching candidate {visibleTargets.length === 1 ? "slot" : "slots"}</span></label><label className="block text-sm font-bold">1. Choose a candidate<select value={selectedKey} onChange={(event) => { setSelectedKey(event.target.value); setFormError(null); setReceipt(null); setResearchPackage(null); }} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2.5 text-sm"><option value="">Select a candidate</option>{visibleTargets.map((target) => <option key={keyFor(target)} value={keyFor(target)}>{target.candidateName} — {target.location}{target.currentPhotoUrl ? " · current image" : " · missing image"}</option>)}</select></label></div><label className="inline-flex cursor-pointer items-center gap-2 text-xs font-semibold text-muted-foreground"><input type="checkbox" checked={showAllCandidates} onChange={(event) => { setShowAllCandidates(event.target.checked); setSelectedKey(""); setCandidateQuery(""); setReceipt(null); setResearchPackage(null); }} /> Include candidates with current images</label></div>
      {!selectedTarget ? <p className="mt-5 rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Select a candidate to prepare source-backed image evidence or review an image.</p> : <div className="mt-5 rounded-xl border border-primary/25 bg-primary/[0.025] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-primary">Selected candidate</p><h3 className="mt-1 text-lg font-bold">{selectedTarget.candidateName}</h3><p className="mt-1 text-sm text-muted-foreground">{selectedTarget.location}</p></div>{selectedPending && <span className="rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-bold text-amber-700 dark:text-amber-300">Ready for your decision</span>}</div>
        ...
        {receipt && <p role="status" className="mt-4 rounded-md border border-emerald-500/30 bg-emerald-500/5 p-3 text-sm font-semibold text-emerald-700 dark:text-emerald-300"><CheckCircle2 className="mr-1 inline" size={15}/>{receipt}</p>}
        {!selectedPending && <div className="mt-4 rounded-lg border border-primary/20 bg-primary/[0.03] p-3"><p className="text-xs font-bold text-foreground">2. Research or provide a source package</p><p className="mt-1 text-xs text-muted-foreground">The research desk checks available evidence and returns a direct image package only when it can support one. If evidence is insufficient, it tells you exactly what to add next. It never publishes an image automatically.</p><button onClick={requestPortraitResearch} disabled={researchNow.isPending} className="mt-2 inline-flex items-center gap-1 rounded-md border border-primary/40 bg-background px-3 py-2 text-xs font-bold text-primary disabled:opacity-50"><Search size={13}/>{researchNow.isPending ? "Checking evidence…" : "Check available evidence"}</button>{researchPackage && <p className={`mt-3 rounded-md border p-2 text-xs font-semibold ${researchPackage.proposals.length ? "border-emerald-500/30 bg-emerald-500/5 text-emerald-700 dark:text-emerald-300" : "border-amber-500/30 bg-amber-500/5 text-amber-700 dark:text-amber-300"}`}>{researchPackage.proposals.length ? "Evidence package ready below. Verify it before sending to Approve / Deny." : "No direct image package was found. Use an official-source lead below or upload a sourced image; your review remains required."}</p>}</div>}
        {selectedPending ? <div className="mt-5 grid gap-4 rounded-xl border border-amber-500/35 bg-background/70 p-4 md:grid-cols-[160px_1fr]">{selectedPreviewUnavailable ? <div className="grid h-40 w-40 max-w-full place-items-center rounded-lg border border-dashed border-destructive/45 bg-destructive/5 p-3 text-center text-xs font-bold text-destructive">Image unavailable<br/>Do not approve</div> : <img src={selectedPending.imageUrl} alt={`Image proposed for ${selectedPending.candidateName}`} onError={() => setSelectedPreviewUnavailable(true)} className="h-40 w-40 max-w-full rounded-lg border border-border object-cover"/>}<div><p className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300">4. Review this proposed image</p><p className="mt-2 text-sm leading-6 text-muted-foreground">Confirm that the person is {selectedPending.candidateName}, the image is appropriate, and the source supports the identity claim.</p><div className="mt-3 flex flex-wrap gap-3 text-xs font-semibold"><a href={selectedPending.imageUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary underline">Open image <ExternalLink size={12}/></a><a href={selectedPending.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary underline">Open source <ExternalLink size={12}/></a></div>{selectedPreviewUnavailable && <p className="mt-3 text-xs font-semibold text-destructive">Preview failed to load. Deny this item with a reason or replace the source package; approval is blocked.</p>}<input value={reviewNotes[selectedPending.id] ?? ""} onChange={(event) => setReviewNotes((notes) => ({ ...notes, [selectedPending.id]: event.target.value }))} placeholder="Optional approval note; required if you deny this image" className="mt-4 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"/><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => decide("approved")} disabled={review.isPending || selectedPreviewUnavailable} className="inline-flex items-center gap-1 rounded-md bg-emerald-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50"><CheckCircle2 size={15}/> Approve image</button><button onClick={() => decide("rejected")} disabled={review.isPending} className="inline-flex items-center gap-1 rounded-md border border-destructive/45 bg-destructive/5 px-4 py-2 text-sm font-bold text-destructive disabled:opacity-50"><XCircle size={15}/> Deny image</button></div></div></div> : <div className="mt-5"><p className="text-xs font-bold uppercase tracking-wider text-primary">3. Add or replace a sourced image</p>{selectedTarget.currentPhotoUrl && <div className="mt-3 flex items-center gap-3 rounded-lg border border-border bg-background/60 p-3"><img src={selectedTarget.currentPhotoUrl} alt={`Current public portrait of ${selectedTarget.candidateName}`} className="h-16 w-16 rounded-md border border-border object-cover"/><p className="text-xs text-muted-foreground">Current public image. A newly approved submission will replace this portrait on the public candidate record.</p></div>}<p className="mt-2 text-sm text-muted-foreground">Upload an image file or submit a direct secure image URL, then add the official page that establishes provenance. Nothing changes publicly until you approve the queued package.</p><div className="mt-3 flex flex-wrap gap-2">{sourceLeads.map((lead) => <a key={lead.label} href={lead.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-3 py-2 text-xs font-bold text-primary"><span>{lead.label}</span><ExternalLink size={11}/></a>)}</div><details className="mt-4 rounded-lg border border-border bg-background/65 p-3"><summary className="cursor-pointer text-sm font-bold">Upload or submit image evidence</summary><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold">Upload portrait file<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => uploadFile(event.target.files?.[0])} className="mt-1 block w-full text-xs" disabled={upload.isPending}/><span className="mt-1 block font-normal text-muted-foreground">JPG, PNG, WEBP, or GIF · maximum 5 MB</span></label><label className="text-xs font-bold">Direct image URL<input value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} placeholder="https://…/portrait.jpg" className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"/></label><label className="text-xs font-bold">Official source page<input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://official-source.example/profile" className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"/></label><label className="text-xs font-bold">Source type<select value={provenanceType} onChange={(event) => setProvenanceType(event.target.value)} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm">{Object.entries(provenanceLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-xs font-bold">Optional note<input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Why this image matches the candidate" className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"/></label></div>{upload.isPending && <p className="mt-3 text-xs font-semibold text-primary">Saving image file to secure storage…</p>}{imageUrl.startsWith("/manus-storage/") && <p className="mt-3 text-xs font-semibold text-emerald-700 dark:text-emerald-300">Image file saved to secure storage. Add provenance, then send it for approval.</p>}<button onClick={sendEvidenceForReview} disabled={submit.isPending || upload.isPending} className="mt-3 inline-flex items-center gap-1 rounded-md border border-primary/45 bg-primary/10 px-4 py-2 text-sm font-bold text-primary disabled:opacity-50"><ImagePlus size={15}/> Send image to Approve / Deny</button></details></div>}</div>}
    </section>
