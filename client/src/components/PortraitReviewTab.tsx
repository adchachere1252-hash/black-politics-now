...
export function PortraitReviewTab({ initialTargetKey }: { initialTargetKey?: string }) {
  ...
  const upload = trpc.portraits.upload.useMutation({
    onSuccess: ({ url }) => { setImageUrl(url); setFormError(null); },
    onError: (error) => setFormError(error.message),
  });
  ...
  return <div className="space-y-5">
    ...
    <section className="glass-card rounded-xl p-5">
      <div className="flex flex-wrap items-end justify-between gap-3"><label className="block flex-1 text-sm font-bold">1. Choose a candidate<select value={selectedKey} onChange={(event) => { setSelectedKey(event.target.value); setFormError(null); }} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2.5 text-sm"><option value="">Select a candidate</option>{managedTargetList.map((target) => <option key={keyFor(target)} value={keyFor(target)}>{target.candidateName} — {target.location}{target.currentPhotoUrl ? " · current image" : " · missing image"}</option>)}</select></label><label className="inline-flex cursor-pointer items-center gap-2 text-xs font-semibold text-muted-foreground"><input type="checkbox" checked={showAllCandidates} onChange={(event) => { setShowAllCandidates(event.target.checked); setSelectedKey(""); }} /> Include candidates with current images</label></div>
