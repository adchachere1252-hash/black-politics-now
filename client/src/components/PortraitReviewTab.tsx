...
export function PortraitReviewTab({ initialTargetKey }: { initialTargetKey?: string }) {
  ...
  return <div className="space-y-5">
    <section className="glass-card rounded-xl p-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 rounded-lg border border-violet-500/25 bg-violet-500/[0.035] p-3"><SearchCheck size={16} className="text-violet-700 dark:text-violet-200"/><p className="mr-auto text-xs leading-5 text-muted-foreground"><strong className="text-foreground">Bulk AI search:</strong> search every remaining image gap in the background. The AI does not publish anything; every found image still appears one at a time for your decision.</p><button onClick={() => { if (window.confirm(`Start private AI image research for all ${targets.length} remaining candidates? Each result will remain private until you approve or deny it.`)) { setFormError(null); startAllResearch.mutate(); } }} disabled={startAllResearch.isPending || !targets.length || latestBatch?.status === "running"} className="inline-flex items-center gap-1 rounded-md border border-violet-500/40 bg-violet-500/10 px-3 py-2 text-xs font-bold text-violet-700 disabled:opacity-50 dark:text-violet-200"><SearchCheck size={14}/>{startAllResearch.isPending ? "Starting bulk search…" : latestBatch?.status === "running" ? "Bulk search running" : `Search all ${targets.length} candidates`}</button></div>
    </section>
