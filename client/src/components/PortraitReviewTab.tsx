...
export function PortraitReviewTab({ initialTargetKey }: { initialTargetKey?: string }) {
  ...
  const researchNow = trpc.portraits.researchNow.useMutation({
    onSuccess: () => {
      setFormError(null);
      setReceipt("Research queued. The evidence package will return to the portrait review workflow when it is ready for your decision.");
    },
    onError: (error) => setFormError(error.message),
  });
  ...
  return <div className="space-y-5">
    <section className="glass-card rounded-xl p-5">
      ...
      <div className="mt-4 rounded-lg border border-primary/25 bg-primary/[0.035] p-3 text-xs leading-5 text-muted-foreground"><strong className="text-foreground">Working method:</strong> search the candidate directory, open a curated official-source lead, copy a direct image URL and the page that establishes provenance, then send the package to the Approve / Deny queue. The platform validates secure links and retains both the image and source for review.</div>
    </section>

    <section className="glass-card rounded-xl p-5">
      <div className="flex flex-wrap items-end justify-between gap-3"><div className="grid min-w-0 flex-1 gap-3 md:grid-cols-[minmax(200px,.75fr)_minmax(260px,1.25fr)]"><label className="block text-sm font-bold">Find a candidate<span className="relative mt-2 block"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"/><input value={candidateQuery} onChange={(event) => { setCandidateQuery(event.target.value); setFormError(null); }} placeholder="Search name, state, district, or office" className="w-full rounded-md border border-border bg-background py-2.5 pl-8 pr-3 text-sm font-normal"/></span><span className="mt-1 block text-[11px] font-normal text-muted-foreground">{visibleTargets.length} matching candidate {visibleTargets.length === 1 ? "slot" : "slots"}</span></label><label className="block text-sm font-bold">1. Choose a candidate<select value={selectedKey} onChange={(event) => { setSelectedKey(event.target.value); setFormError(null); setReceipt(null); }} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2.5 text-sm"><option value="">Select a candidate</option>{visibleTargets.map((target) => <option key={keyFor(target)} value={keyFor(target)}>{target.candidateName} — {target.location}{target.currentPhotoUrl ? " · current image" : " · missing image"}</option>)}</select></label></div><label className="inline-flex cursor-pointer items-center gap-2 text-xs font-semibold text-muted-foreground"><input type="checkbox" checked={showAllCandidates} onChange={(event) => { setShowAllCandidates(event.target.checked); setSelectedKey(""); setCandidateQuery(""); setReceipt(null); }} /> Include candidates with current images</label></div>
