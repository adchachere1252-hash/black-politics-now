...
export function AgentProposedChangesTab() {
  ...
  return <div className="space-y-6">
    ...
    {isLoading ? <div className="h-52 animate-pulse rounded-xl bg-muted" /> : visible.length === 0 ? <div className="rounded-xl border border-dashed border-border p-10 text-center"><FileDiff className="mx-auto text-muted-foreground" size={30}/><h3 className="mt-3 font-semibold">No {status === "all" ? "agent change sets" : statusLabels[status]?.toLowerCase() || "matching change sets"} yet</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">The <strong className="text-foreground">Run fresh research</strong> action appears on a proposal after an Agent Desk recommendation is approved as an agent task and run. That private work package can submit up to three evidence-backed proposals here for your review.</p><a href="/admin?tab=agent" className="mt-4 inline-flex rounded-md bg-primary px-3 py-2 text-xs font-bold text-primary-foreground">Open Agent Desk to start research</a></div> : <div className="space-y-4">{visible.map((proposal) => {
