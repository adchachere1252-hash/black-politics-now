...
import { Bot, CheckCircle2, Clock3, FileCheck2, ListTodo, Play, RefreshCw, Sparkles, UserRound, XCircle, Zap } from "lucide-react";
...
const taskStatuses = ["open", "in_progress", "blocked", "ready_for_review", "completed"] as const;
...
export function AgentDeskTab({ focusRecommendationId }: { focusRecommendationId?: number }) {
  ...
  const [executionModeDrafts, setExecutionModeDrafts] = useState<Record<number, "human" | "agent">>({});
  const [executionScopeDrafts, setExecutionScopeDrafts] = useState<Record<number, string>>({});
  const [sourceRequirementDrafts, setSourceRequirementDrafts] = useState<Record<number, string>>({});
  ...
  const executeTask = trpc.agent.executeTask.useMutation({ onSuccess: invalidateDesk });
  ...
  return (
    <div className="space-y-6">
        ...
        {(recommendations as any[]).length === 0 ? (
          ...
          <div className="space-y-3">
            {(recommendations as any[]).map((item) => {
              ...
              const executionMode = executionModeDrafts[item.id] ?? "human";
              const executionScope = executionScopeDrafts[item.id] ?? "";
              const sourceRequirements = sourceRequirementDrafts[item.id] ?? "";
              return <article id={`agent-recommendation-${item.id}`} key={item.id} className={`rounded-xl border bg-card p-4 transition-colors ${focusRecommendationId === item.id ? "border-primary ring-2 ring-primary/20" : "border-border"}`}>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  {item.status === "pending" && <div className="flex w-full shrink-0 flex-col gap-2 sm:w-64"><div className="flex gap-1"><input value={draftOwner} onChange={(event) => setOwnerDrafts((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="Assign owner" className="min-w-0 flex-1 rounded-md border border-border bg-background px-2 py-1.5 text-xs" /><button onClick={() => draftOwner.trim() && assign.mutate({ id: item.id, owner: draftOwner.trim() })} disabled={!draftOwner.trim() || assign.isPending} className="rounded-md bg-muted px-2 py-1.5 text-xs font-medium text-muted-foreground">Assign</button></div><input type="date" aria-label="Task due date" value={draftDueDate} onChange={(event) => setDueDateDrafts((current) => ({ ...current, [item.id]: event.target.value }))} className="rounded-md border border-border bg-background px-2 py-1.5 text-xs" /><select aria-label="Task execution mode" value={executionMode} onChange={(event) => setExecutionModeDrafts((current) => ({ ...current, [item.id]: event.target.value as "human" | "agent" }))} className="rounded-md border border-border bg-background px-2 py-1.5 text-xs"><option value="human">Human follow-up</option><option value="agent">Research Desk work package</option></select>{executionMode === "agent" && <><textarea value={executionScope} onChange={(event) => setExecutionScopeDrafts((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="Bounded task scope (what the agent should complete)" rows={3} className="resize-y rounded-md border border-border bg-background px-2 py-1.5 text-xs" /><textarea value={sourceRequirements} onChange={(event) => setSourceRequirementDrafts((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="Required sources or verification standard (optional)" rows={2} className="resize-y rounded-md border border-border bg-background px-2 py-1.5 text-xs" /></>}<button onClick={() => approveToTask.mutate({ id: item.id, ...(draftOwner.trim() ? { owner: draftOwner.trim() } : {}), ...(draftDueDate ? { dueDate: draftDueDate } : {}), executionMode, ...(executionScope.trim() ? { executionScope: executionScope.trim() } : {}), ...(sourceRequirements.trim() ? { sourceRequirements: sourceRequirements.trim() } : {}) })} disabled={approveToTask.isPending} className="inline-flex items-center justify-center gap-1 rounded-md bg-emerald-500/15 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300">{executionMode === "agent" ? <Bot size={13} /> : <ListTodo size={13} />}{executionMode === "agent" ? "Approve & assign agent" : "Approve & task"}</button><div className="flex gap-2"><button onClick={() => review.mutate({ id: item.id, status: "deferred" })} className="flex-1 rounded-md bg-muted px-2 py-1.5 text-xs font-semibold text-muted-foreground">Defer</button><button onClick={() => review.mutate({ id: item.id, status: "dismissed" })} className="flex-1 rounded-md bg-red-500/10 px-2 py-1.5 text-xs font-semibold text-red-700 dark:text-red-300">Dismiss</button></div></div>}
                </div>
              </article>;
            })}
          </div>
      ...
      <section><h3 className="mb-1 font-bold">Approved follow-up tasks</h3><p className="mb-3 text-xs text-muted-foreground">Agent-assigned tasks create only cited, review-ready work packages. A human still decides whether anything is published or entered into platform records.</p><div className="space-y-3">{(tasks as any[]).slice(0, 8).map((task) => { const taskDueDate = task.dueDate ? new Date(task.dueDate).toISOString().slice(0, 10) : ""; const isAgentTask = task.executionMode === "agent"; const canRun = isAgentTask && ["open", "blocked"].includes(task.status); return <div key={task.id} className="rounded-lg border border-border px-4 py-3 text-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-medium">{task.title}</span>{task.owner && <span className="text-muted-foreground">· {task.owner}</span>}<span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${isAgentTask ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>{isAgentTask ? "Agent package" : "Human task"}</span>{task.status === "ready_for_review" && <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold uppercase text-emerald-700 dark:text-emerald-300"><FileCheck2 size={11} />Ready for review</span>}</div><p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{task.description}</p>{isAgentTask && <p className="mt-2 text-xs text-muted-foreground"><strong className="text-foreground">Scope:</strong> {task.executionScope || "Source-grounded research and analysis package."}</p>}{task.executionError && <p className="mt-2 text-xs text-destructive">Last execution: {task.executionError}</p>}</div><div className="flex flex-wrap gap-2"><input type="date" aria-label="Update task due date" defaultValue={taskDueDate} onBlur={(event) => { if (event.target.value !== taskDueDate) updateTask.mutate({ id: task.id, status: task.status as typeof taskStatuses[number], ...(event.target.value ? { dueDate: event.target.value } : {}) }); }} className="rounded-md border border-border bg-background px-2 py-1 text-xs" /><select value={task.status} onChange={(event) => updateTask.mutate({ id: task.id, status: event.target.value as typeof taskStatuses[number], ...(taskDueDate ? { dueDate: taskDueDate } : {}) })} className="rounded-md border border-border bg-background px-2 py-1 text-xs">{taskStatuses.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select>{canRun && <button onClick={() => executeTask.mutate({ id: task.id })} disabled={executeTask.isPending} className="inline-flex items-center gap-1 rounded-md bg-primary px-2.5 py-1 text-xs font-semibold text-primary-foreground disabled:opacity-50"><Play size={12} />{executeTask.isPending ? "Working…" : "Run agent task"}</button>}</div></div>{task.agentWorkPackage && <details className="mt-3 rounded-md border border-primary/15 bg-primary/5 p-3"><summary className="cursor-pointer text-xs font-semibold text-primary">Review cited agent work package</summary><div className="mt-3 whitespace-pre-wrap text-xs leading-5 text-muted-foreground">{task.agentWorkPackage}</div></details>}</div>; })}{(tasks as any[]).length === 0 && <p className="text-sm text-muted-foreground">Approved recommendations will appear here as private follow-up tasks.</p>}</div>{executeTask.error && <p className="mt-3 text-sm text-destructive">Agent task execution did not complete. No public content or election data was changed.</p>}</section>
