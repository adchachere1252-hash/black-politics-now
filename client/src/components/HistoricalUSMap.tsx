...
export function HistoricalUSMap({ congress, selectedState, onStateSelect, overlayMode = "party", label, compact = false, onFrameStatus }: { congress: number; selectedState?: string | null; onStateSelect: (stateCode: string) => void; overlayMode?: AtlasOverlayMode; label?: string; compact?: boolean; onFrameStatus?: (status: AtlasFrameStatus) => void }) {
  ...
  return <div className="relative overflow-hidden rounded-2xl border border-primary/25 bg-[radial-gradient(circle_at_53%_30%,rgba(225,173,83,.13),transparent_42%),var(--background)] shadow-[inset_0_0_54px_rgba(225,173,83,.08)]">
    <div className="absolute left-4 top-4 z-10 max-w-[72%] rounded border border-border bg-card/95 px-2.5 py-1.5 text-[10px] text-muted-foreground backdrop-blur-sm"><strong className="text-primary">{map.renderedStates}/50</strong> states loaded · <strong className="text-primary">435</strong> apportioned House seats · <span>{map.sourceGeometryCount || "—"} mapped district regions</span>{loadingFrame && frame && <span className="ml-2 text-primary">· Loading {congress}th frame</span>}</div>
    ...
    {hover && <div className="pointer-events-none absolute z-20 max-w-64 rounded border border-primary/30 bg-popover px-2.5 py-2 text-[10px] text-popover-foreground shadow-xl" style={{ left: Math.min(hover.x + 12, 760), top: Math.max(40, hover.y - 62) }}><strong className="block text-primary">{hover.path.state} · {hover.path.district}</strong>{hover.path.member ? <><span className="block font-semibold text-foreground">{hover.path.member.name}</span><span className="text-muted-foreground">{hover.path.member.party === "D" ? "Democratic Party" : hover.path.member.party === "R" ? "Republican Party" : `Other party · ICPSR ${hover.path.member.partyCode}`}</span></> : <span className="text-muted-foreground">{overlayMode === "boundary" ? "Validated UCLA congressional boundary geometry" : "No verified House member match in the source export"}</span>}</div>}
    <div className="absolute bottom-4 left-4 z-10 flex max-w-[92%] flex-wrap gap-x-3 gap-y-1 rounded border border-border bg-card/95 px-2.5 py-1.5 text-[9px] text-muted-foreground backdrop-blur-sm">{legend.map((item) => <span key={item.label} className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-sm" style={{ backgroundColor: item.color }} />{item.label}</span>)}{overlayMode === "member" && <span>Hover a district for verified roster context</span>}<span className="basis-full text-muted-foreground/85">District geometry: UCLA Congressional District Maps. Seat total: Census apportionment. A single at-large region may represent more than one House seat.</span></div>
