...
export default function HomepageExample({ mode = "preview" }: { mode?: "preview" | "home" }) {
  ...
  const worldBrief = useMemo(() => {
    ...
    const regions = new Set(records.map((item) => item.region).filter(Boolean)).size;
    return { upcoming, live: records.filter((item) => item.status === "Voting Today").length, total: records.length, regions };
  }, [worldElections]);
