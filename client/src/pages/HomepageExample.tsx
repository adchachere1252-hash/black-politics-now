...
export default function HomepageExample({ mode = "preview" }: { mode?: "preview" | "home" }) {
  ...
  const { data: primaryContexts = [] } = trpc.election.primaryContexts.useQuery(undefined, homepageElectionQueryOptions);
  ...
  const selectedPrimaryContext = useMemo(() => (primaryContexts as any[]).find((item) => item.stateCode === selectedState), [primaryContexts, selectedState]);
  const alaskaPrimaryContext = useMemo(() => (primaryContexts as any[]).find((item) => item.stateCode === "AK"), [primaryContexts]);
  ...
  const mapLabel = mapMode === "senate" ? "2026 U.S. Senate Outlook" : mapMode === "house" ? "2026 U.S. House Outlook" : mapMode === "governor" ? "2026 Governor Outlook" : "Black Political Representation Tracker";
  const stateTitle = selectedState ? `${STATE_NAMES[selectedState] ?? selectedState} · ${mapMode === "house" ? "House races" : mapMode === "governor" ? "Governor race" : mapMode === "senate" ? "Senate race" : "Black Political Representation"}` : "State race details";
  ...
  return (
    <div className="homepage-editorial-home homepage-atlas-shell hidden h-[calc(100dvh-64px)] overflow-hidden bg-background px-2 pb-0 pt-2 lg:block">
      <main className="homepage-atlas-main mx-auto flex h-full max-w-[1640px] min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card p-2 shadow-[0_18px_70px_rgba(17,24,39,0.12)] dark:shadow-black/35">
        ...
        <div className="mb-2 shrink-0"><ResultsTicker senateRaces={senateRaces as any[] ?? []} houseRaces={houseRaces as any[] ?? []} tickerEntries={tickerEntries as any[] ?? []} /></div>
        <section className="grid min-h-0 flex-1 grid-cols-[0.92fr_1.68fr_0.96fr] grid-rows-[minmax(0,1fr)] gap-2">
          ...
          <section className="homepage-atlas-panel relative min-h-0 overflow-hidden rounded-lg border border-border bg-background/55">
            ...
            <div className="pointer-events-none absolute inset-x-3 top-3 z-10 rounded-lg border border-border bg-card/94 p-3 shadow-sm"><div className="pointer-events-auto flex flex-wrap items-start justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Interactive Election Map</p><h1 className="mt-0.5 text-[clamp(1rem,1.55vw,1.3rem)] font-bold tracking-tight text-foreground">{mapLabel}</h1><p className="mt-0.5 text-[11px] text-muted-foreground">Choose a general-election view, then select a state for verified details and notes.</p></div><div className="flex rounded border border-border bg-background/85 p-0.5 shadow-sm">{(["blackrep", "governor", "house", "senate"] as MapMode[]).map((item) => <button key={item} type="button" onClick={() => { setMapMode(item); setSelectedState(null); setMapSearch(""); }} className={`rounded px-2 py-1 text-[8px] font-bold uppercase tracking-[0.09em] transition-colors ${mapMode === item ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>{item === "blackrep" ? "Black Reps" : item}</button>)}</div></div><div className="pointer-events-auto relative mt-2 max-w-sm"><Search className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground" size={12} /><input value={mapSearch} onChange={(event) => setMapSearch(event.target.value)} placeholder="Search a state or person" className="h-7 w-full rounded border border-border bg-background/85 pl-7 pr-2 text-[10px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/60" aria-label="Search election map" />{mapSearchMatches.length > 0 && <div className="absolute inset-x-0 top-8 overflow-hidden rounded border border-border bg-card shadow-lg">{mapSearchMatches.map((match) => <button key={match.stateCode} type="button" onClick={() => { setSelectedState(match.stateCode); setStateDetailOpen(true); setMapSearch(""); }} className="block w-full border-b border-border px-2 py-1.5 text-left text-[9px] text-foreground last:border-b-0 hover:bg-muted">{match.label}</button>)}</div>}</div>{alaskaPrimaryContext && <button type="button" onClick={() => { setSelectedState("AK"); setStateDetailOpen(true); }} className="pointer-events-auto mt-2 inline-flex max-w-full items-center gap-1 rounded border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-left text-[8px] font-semibold text-amber-800 hover:bg-amber-500/15 dark:text-amber-200"><span className="uppercase tracking-[.09em]">Primary context</span><span className="text-amber-700/75 dark:text-amber-100/75">· Alaska reviewed {alaskaPrimaryContext.reviewedAt}</span></button>}<div className="mt-2 flex flex-wrap gap-x-2.5 gap-y-1">{mapMode === "blackrep" ? <span className="inline-flex items-center gap-1 text-[8px] text-muted-foreground"><i className="h-2 w-2 rounded-full bg-[var(--color-representation)]" />Black Political Representation presence</span> : LEGEND.map(([label, color]) => <span key={label} className="inline-flex items-center gap-1 text-[8px] text-muted-foreground"><i className={`h-2 w-2 rounded-full ${color}`} />{label}</span>)}</div></div>
      ...
      <Dialog open={stateDetailOpen} onOpenChange={setStateDetailOpen}>
        <DialogContent className="max-h-[82vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{stateTitle}</DialogTitle></DialogHeader><div className="space-y-2">{selectedPrimaryContext && <section className="rounded-lg border border-amber-500/35 bg-amber-500/10 p-3 text-xs text-foreground"><p className="font-bold uppercase tracking-[.1em] text-amber-800 dark:text-amber-200">{selectedPrimaryContext.title} · {selectedPrimaryContext.primaryDate}</p><p className="mt-1 leading-relaxed text-muted-foreground">{selectedPrimaryContext.summary}</p><p className="mt-2 font-medium text-foreground">General-election boundary: <span className="font-normal text-muted-foreground">{selectedPrimaryContext.generalElectionBoundary}</span></p><a href={selectedPrimaryContext.sourceUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex font-semibold text-primary hover:underline">{selectedPrimaryContext.sourceLabel} ↗</a></section>}{selectedRaces.length ? selectedRaces.map((race, index) => mapMode === "blackrep" ? <BlackRepresentationDetail key={`${race.popupType}-${race.id ?? index}`} record={race} /> : <RaceDetail key={race.id ?? index} race={race} mode={mapMode} />) : <p className="rounded border border-border p-3 text-sm text-muted-foreground">No {mapMode === "blackrep" ? "Black Representation profiles or article-backed election records" : mapMode === "house" ? "House races" : mapMode === "governor" ? "Governor race" : "Senate race"} are currently tracked for this state.</p>}</div></DialogContent>
      </Dialog>
    </div>
  );
}
