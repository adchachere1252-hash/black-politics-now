      ...
      <main className="mx-auto flex h-full max-w-[1640px] min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card p-2 shadow-[0_18px_70px_rgba(17,24,39,0.12)] dark:shadow-black/35">
        ...
        <section className="grid min-h-0 flex-1 grid-cols-[0.92fr_1.68fr_0.96fr] grid-rows-[minmax(0,1fr)] gap-2">
          <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background/55 p-3">
            ...
            <Link href="/atlas" className="group relative mt-2 h-[clamp(164px,24vh,216px)] shrink-0 overflow-hidden rounded-lg border border-primary/35 bg-card transition-colors hover:bg-muted/50"><img src="/manus-storage/selma-marchers-homepage_4dbcaa12.jpg" alt="Civil rights marchers crossing the Edmund Pettus Bridge in Selma" className="absolute inset-0 h-full w-full object-cover opacity-30 mix-blend-multiply transition-transform duration-500 group-hover:scale-105 dark:opacity-45 dark:mix-blend-luminosity" /><div className="absolute inset-0 bg-gradient-to-t from-card via-card/80 to-card/20 dark:from-card dark:via-card/75 dark:to-background/20" /><div className="relative z-10 flex h-full flex-col justify-end p-3"><div className="flex items-start justify-between gap-2"><p className="text-[8px] font-bold uppercase tracking-[0.15em] text-primary"><Landmark className="mr-1 inline" size={10} />Voting Rights Act</p><ArrowUpRight className="text-primary" size={11} /></div><p className="mt-1 text-[8px] font-semibold uppercase tracking-[0.11em] text-muted-foreground">Selma · 1965</p><h3 className="mt-1 text-[15px] font-bold leading-[1.05] text-foreground">The map has a memory.</h3><p className="mt-1 max-w-[90%] text-[8px] leading-snug text-muted-foreground">A living atlas of the VRA, representation, apportionment, and today’s redistricting fights.</p><div className="mt-2 grid grid-cols-3 divide-x divide-border rounded border border-border bg-background/80 text-center backdrop-blur-[1px]"><AtlasStat value="1965" label="VRA" /><AtlasStat value={atlasBrief.tracked} label="States" /><AtlasStat value={atlasBrief.litigated} label="Cases" /></div></div></Link>
          </aside>

          <section className="relative min-h-0 overflow-hidden rounded-lg border border-border bg-background/55">
            <div className="absolute inset-0"><USMapFull raceData={mapData} selectedState={selectedState} onStateClick={(state) => { setSelectedState(state); setStateDetailOpen(true); }} />{mapLoading && <div className="absolute inset-0 grid place-items-center bg-background/45 backdrop-blur-[1px]"><div className="rounded-full border border-primary/25 bg-card/90 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary shadow-sm">Loading election intelligence</div></div>}</div>
            <div className="pointer-events-none absolute inset-x-0 top-0 z-10 bg-gradient-to-b from-background/95 via-background/80 to-transparent p-3 pb-9"><div className="pointer-events-auto flex flex-wrap items-start justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Interactive Election Map</p><h1 className="mt-0.5 text-[clamp(1rem,1.55vw,1.3rem)] font-bold tracking-tight text-foreground">{mapLabel}</h1><p className="mt-0.5 text-[11px] text-muted-foreground">Choose a chamber, then select a state for contest notes and detail.</p></div><div className="flex rounded border border-border bg-card/95 p-0.5 shadow-sm">{(["senate", "house", "governor"] as MapMode[]).map((item) => <button key={item} type="button" onClick={() => { setMapMode(item); setSelectedState(null); }} className={`rounded px-2 py-1 text-[8px] font-bold uppercase tracking-[0.09em] transition-colors ${mapMode === item ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>{item}</button>)}</div></div><div className="mt-2 flex flex-wrap gap-x-2.5 gap-y-1">{LEGEND.map(([label, color]) => <span key={label} className="inline-flex items-center gap-1 text-[8px] text-muted-foreground"><i className={`h-2 w-2 rounded-full ${color}`} />{label}</span>)}</div></div>
            <div className="absolute inset-x-0 bottom-0 z-10 flex items-center gap-2 bg-gradient-to-t from-background/95 via-background/80 to-transparent px-3 pb-2 pt-9 text-[9px]"><span className="font-bold uppercase tracking-[0.09em] text-primary">Live Results</span><span className="text-muted-foreground">•</span><span className="text-muted-foreground">Race calls, reporting, and new polling</span><Link href="/elections" className="ml-auto inline-flex items-center gap-1 font-semibold text-primary">More <ArrowUpRight size={10} /></Link></div>
          </section>
          ...
          <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background/55 p-3"><h2 className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Daily Intelligence Brief</h2>
            ...
            <Link href="/world" className="group relative mt-2 h-[clamp(164px,24vh,216px)] shrink-0 overflow-hidden rounded-lg border border-cyan-300/35 bg-[radial-gradient(circle_at_77%_36%,rgba(121,221,255,0.58),transparent_42%),radial-gradient(circle_at_12%_118%,rgba(76,102,230,0.5),transparent_54%),linear-gradient(135deg,#041124,#0a315a_52%,#11213b)] p-3 shadow-[inset_0_0_44px_rgba(94,199,255,0.18)] transition-all hover:border-cyan-200 hover:shadow-[inset_0_0_50px_rgba(94,199,255,0.24)]"><div className="relative z-10 flex h-full max-w-[56%] flex-col justify-end"><div className="flex items-start justify-between gap-2"><p className="text-[8px] font-bold uppercase tracking-[0.15em] text-cyan-50"><Globe2 className="mr-1 inline" size={10} />World Elections</p><span className="rounded border border-cyan-100/30 bg-cyan-100/10 px-1.5 py-0.5 text-[8px] font-bold text-cyan-50">{worldBrief.total}</span></div><h3 className="mt-1 text-[16px] font-bold leading-[1.05] text-white">Democracy is moving.</h3><p className="mt-1 text-[9px] leading-snug text-cyan-100/80">A live field guide to elections, transitions, and civic power beyond U.S. borders.</p><div className="mt-2 grid grid-cols-3 divide-x divide-cyan-100/20 rounded border border-cyan-100/20 bg-slate-950/25 text-center"><WorldStat value={worldBrief.total} label="Tracked" /><WorldStat value={worldBrief.upcomingCount} label="Upcoming" /><WorldStat value={worldBrief.live || "—"} label="Live" /></div>{worldBrief.upcoming && <p className="mt-1.5 truncate text-[8px] font-semibold text-cyan-50/90">Next: {worldBrief.upcoming.country} · {shortDate(worldBrief.upcoming.electionDate)}</p>}</div><div aria-hidden className="absolute -bottom-5 -right-5 h-[190px] w-[190px] drop-shadow-[0_0_30px_rgba(110,222,255,0.98)] transition-transform duration-500 group-hover:scale-105"><MiniRepositoryGlobe theme={theme} vibrant /></div></Link>
          </aside>
        </section>
      </main>
