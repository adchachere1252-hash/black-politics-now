      ...
      <main className="mx-auto flex h-full max-w-[1640px] min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card p-2 shadow-[0_18px_70px_rgba(17,24,39,0.12)] dark:shadow-black/35">
        ...
        <section className="grid min-h-0 flex-1 grid-cols-[0.92fr_1.68fr_0.96fr] grid-rows-[minmax(0,1fr)] gap-2">
          <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background/55 p-3">
            ...
            <Link href="/atlas" className="group relative mt-2 h-[clamp(164px,24vh,216px)] shrink-0 overflow-hidden rounded-lg border border-primary/35 bg-card transition-colors hover:bg-muted/50"><img src="/manus-storage/selma-marchers-homepage_4dbcaa12.jpg" alt="Civil rights marchers crossing the Edmund Pettus Bridge in Selma" className="absolute inset-y-0 left-0 h-full w-[45%] object-cover opacity-90 transition-transform duration-500 group-hover:scale-105 dark:opacity-95" /><div className="absolute inset-y-0 left-[36%] w-[20%] bg-gradient-to-r from-transparent via-card/65 to-card dark:via-card/55" /><div className="relative z-10 ml-[41%] flex h-full flex-col justify-end p-3"><div className="flex items-start justify-between gap-2"><p className="text-[8px] font-bold uppercase tracking-[0.15em] text-primary"><Landmark className="mr-1 inline" size={10} />Voting Rights Act</p><ArrowUpRight className="text-primary" size={11} /></div><p className="mt-1 text-[8px] font-semibold uppercase tracking-[0.11em] text-muted-foreground">Selma · 1965</p><h3 className="mt-1 text-[15px] font-bold leading-[1.05] text-foreground">The map has a memory.</h3><p className="mt-1 text-[8px] leading-snug text-muted-foreground">A living atlas of the VRA, representation, apportionment, and today’s redistricting fights.</p><div className="mt-2 grid grid-cols-3 divide-x divide-border rounded border border-border bg-background/85 text-center"><AtlasStat value="1965" label="VRA" /><AtlasStat value={atlasBrief.tracked} label="States" /><AtlasStat value={atlasBrief.litigated} label="Cases" /></div></div></Link>
          </aside>
          ...
          <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background/55 p-3">
            ...
            <div className="relative mt-1 min-h-0 flex-1 overflow-hidden"><USMapFull raceData={mapData} selectedState={selectedState} onStateClick={(state) => { setSelectedState(state); setStateDetailOpen(true); }} />{mapLoading && <div className="absolute inset-0 grid place-items-center bg-background/45 backdrop-blur-[1px]"><div className="rounded-full border border-primary/25 bg-card/90 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary shadow-sm">Loading election intelligence</div></div>}</div>
          ...
          <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background/55 p-3"><h2 className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Daily Intelligence Brief</h2>
            <div className="mt-2 grid grid-cols-[76px_1fr] gap-2.5 border-b border-border pb-2.5"><div className="flex aspect-[0.8] items-end rounded border border-primary/45 bg-[linear-gradient(145deg,var(--secondary),var(--background))] p-2"><div><p className="text-sm font-black leading-[0.8] text-foreground">BLACK<br />POLITICS<br />NOW</p><p className="mt-2 border-t border-primary/70 pt-1 text-[7px] font-bold tracking-[0.1em] text-primary">DAILY BRIEF</p></div></div><div><p className="text-[10px] text-muted-foreground">{latestEpisode?.day ?? "Today"} {latestEpisode?.date ? `· ${latestEpisode.date}` : ""}</p><h3 className="mt-1 text-base font-bold leading-tight text-foreground">The Daily Intelligence Brief</h3><p className="mt-1.5 text-[10px] text-muted-foreground">{episodesLoading ? "Loading verified briefing" : latestEpisode?.totalDurationLabel ? `${latestEpisode.totalDurationLabel} · Current analysis and context` : "Current analysis and context"}</p></div></div>
