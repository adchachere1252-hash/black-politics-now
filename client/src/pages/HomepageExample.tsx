...
import { ArrowUpRight, Download, Globe2, Landmark, Mic2, Play, Search } from "lucide-react";
...
import { AUGUST_18_PRIMARY_RESULTS, buildPrimaryResultsMapData, type PrimaryResultRecord } from "@/lib/primaryResultsMap";

type MapMode = "senate" | "house" | "governor" | "blackrep" | "primary";
...
function PrimaryResultsDetail({ record }: { record: PrimaryResultRecord }) {
  return <article className="rounded-md border border-[color:var(--color-primary-result)]/45 bg-card p-3">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-[8px] font-bold uppercase tracking-[0.13em] text-[color:var(--color-primary-result)]">Primary Results · Aug. 18</p><h4 className="mt-0.5 text-sm font-bold text-foreground">{record.stateName}</h4><p className="text-[10px] text-muted-foreground">{record.electionLabel}</p></div><span className="rounded border border-[color:var(--color-primary-result)]/35 bg-[color:var(--color-primary-result)]/10 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide text-[color:var(--color-primary-result)]">{record.resultStatus}</span></div>
    <p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">{record.summary}</p>
    <div className="mt-2 divide-y divide-border rounded border border-border bg-muted/20">{record.results.map((result) => <div key={result.label} className="px-2.5 py-2"><p className="text-[8px] font-bold uppercase tracking-[0.1em] text-[color:var(--color-primary-result)]">{result.label}</p><p className="mt-0.5 text-[10px] leading-relaxed text-foreground">{result.outcome}</p></div>)}</div>
    <div className="mt-2 flex flex-wrap items-center gap-2 text-[9px] text-muted-foreground"><span>{record.updatedLabel}</span><a href={record.sourceUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-primary hover:underline">{record.sourceLabel} ↗</a></div>
  </article>;
}
...
export default function HomepageExample({ mode = "preview" }: { mode?: "preview" | "home" }) {
  ...
  const { play, voicePreference, setVoicePreference, currentTrack: selectedAudioTrack, progress, duration } = useAudio();
  ...
  const currentTrack = selectedAudioTrack ?? { episodeDate: "", title: "", segmentKey: undefined, segmentRole: undefined, segmentOrdinal: undefined, segmentTotal: undefined };
  const mapLoading = mapMode === "senate" ? senateLoading : mapMode === "house" ? houseLoading : mapMode === "governor" ? governorLoading : mapMode === "blackrep" ? blackRepresentationLoading : false;
  ...
  const mapData = useMemo(() => {
    ...
    } else if (mapMode === "blackrep") {
      Object.assign(entries, buildBlackPoliticalRepresentationMapData(cbcMembers as any[] ?? []));
    } else {
      Object.assign(entries, buildPrimaryResultsMapData());
    }
    return entries;
  ...
  const selectedRaces = useMemo(() => {
    ...
    if (mapMode === "primary") return AUGUST_18_PRIMARY_RESULTS.filter((record) => record.stateCode === selectedState);
  ...
  const chamberInsight = useMemo(() => {
    if (mapMode === "blackrep") {
      ...
      return { title: "Black Political Representation tracker", coverage: "representation profiles", total: members.length, tossUps: electionRecords.length, leans: states.size, calls: completed, summary: `${members.length} tracked profiles across ${states.size} states, alongside ${electionRecords.length} article-backed election records.` };
    }
    if (mapMode === "primary") {
      const projected = AUGUST_18_PRIMARY_RESULTS.filter((record) => record.resultStatus === "Projected primary result").length;
      const unofficial = AUGUST_18_PRIMARY_RESULTS.filter((record) => record.resultStatus === "Unofficial special-election result").length;
      const ongoing = AUGUST_18_PRIMARY_RESULTS.filter((record) => record.resultStatus === "Ongoing top-four count").length;
      return { title: "August 18 Primary Results", coverage: "source-backed jurisdictions", total: AUGUST_18_PRIMARY_RESULTS.length, tossUps: projected, leans: unofficial, calls: ongoing, summary: `${projected} projected primary results, ${unofficial} unofficial special-election return, and ${ongoing} ongoing count.` };
    }
  ...
  const mapLabel = mapMode === "senate" ? "2026 U.S. Senate Outlook" : mapMode === "house" ? "2026 U.S. House Outlook" : mapMode === "governor" ? "2026 Governor Outlook" : mapMode === "blackrep" ? "Black Political Representation Tracker" : "August 18 Primary Results";
  const stateTitle = selectedState ? `${STATE_NAMES[selectedState] ?? selectedState} · ${mapMode === "house" ? "House races" : mapMode === "governor" ? "Governor race" : mapMode === "senate" ? "Senate race" : mapMode === "blackrep" ? "Black Political Representation" : "Primary or special-election result"}` : "State race details";
  const mapSearchMatches = useMemo(() => {
    const query = mapSearch.trim().toLowerCase();
    if (query.length < 2) return [];
    const races = (mapMode === "primary" ? AUGUST_18_PRIMARY_RESULTS : mapMode === "blackrep" ? [...(cbcMembers as any[] ?? []), ...(blackRepresentationElections as any[] ?? [])] : mapMode === "senate" ? senateRaces : mapMode === "house" ? houseRaces : governors) as any[] ?? [];
    const seen = new Set<string>();
    return races.flatMap((race) => {
      const stateCode = race.stateCode;
      const stateName = race.stateName ?? STATE_NAMES[stateCode] ?? stateCode;
      const candidates = mapMode === "primary" ? [race.candidate1, race.candidate2, race.electionLabel] : mapMode === "blackrep" ? [race.member, race.winnerName, race.runnerUpName] : mapMode === "governor" ? [race.demCandidate, race.repCandidate] : [race.candidate1Name, race.candidate2Name];
      ...
      seen.add(stateCode);
      return [{ stateCode, label: `${stateName} · ${mapMode === "primary" ? race.resultStatus : mapMode === "house" ? `${(houseRaces as any[] ?? []).filter((item) => item.stateCode === stateCode).length} districts` : mapMode === "blackrep" ? `${(cbcMembers as any[] ?? []).filter((item) => item.stateCode === stateCode).length} members` : candidates.filter(Boolean).join(" vs ")}` }];
    }).slice(0, 6);
  }, [mapSearch, mapMode, senateRaces, houseRaces, governors, cbcMembers, blackRepresentationElections]);

  return (
    <div className="homepage-editorial-home homepage-atlas-shell hidden h-[calc(100dvh-64px)] overflow-hidden bg-background px-2 pb-0 pt-2 lg:block">
      <main className="homepage-atlas-main mx-auto flex h-full max-w-[1640px] min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card p-2 shadow-[0_18px_70px_rgba(17,24,39,0.12)] dark:shadow-black/35">
        ...
        <div className="mb-2 shrink-0 space-y-1"><ResultsTicker senateRaces={senateRaces as any[] ?? []} houseRaces={houseRaces as any[] ?? []} /><ElectionNightStatusStrip compact /></div>
        <section className="grid min-h-0 flex-1 grid-cols-[0.92fr_1.68fr_0.96fr] grid-rows-[minmax(0,1fr)] gap-2">
          ...
          <section className="homepage-atlas-panel relative min-h-0 overflow-hidden rounded-lg border border-border bg-background/55">
            <div className="absolute inset-x-4 bottom-[102px] top-[142px] flex items-center justify-center overflow-hidden rounded-md"><div className="w-[84%] max-w-[680px] translate-y-2"><USMapFull showLegend={false} raceData={mapData} selectedState={selectedState} onStateClick={(state) => { setSelectedState(state); setStateDetailOpen(true); }} /></div>{mapLoading && <div className="absolute inset-0 grid place-items-center bg-background/45 backdrop-blur-[1px]"><div className="rounded-full border border-primary/25 bg-card/90 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary shadow-sm">Loading election intelligence</div></div>}</div>
            <div className="pointer-events-none absolute inset-x-3 top-3 z-10 rounded-lg border border-border bg-card/94 p-3 shadow-sm"><div className="pointer-events-auto flex flex-wrap items-start justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Interactive Election Map</p><h1 className="mt-0.5 text-[clamp(1rem,1.55vw,1.3rem)] font-bold tracking-tight text-foreground">{mapLabel}</h1><p className="mt-0.5 text-[11px] text-muted-foreground">{mapMode === "primary" ? "Source-backed primary and special-election outcomes; not general-election calls." : "Choose a view, then select a state for verified details and notes."}</p></div><div className="flex rounded border border-border bg-background/85 p-0.5 shadow-sm">{(["primary", "blackrep", "governor", "house", "senate"] as MapMode[]).map((item) => <button key={item} type="button" onClick={() => { setMapMode(item); setSelectedState(null); setMapSearch(""); }} className={`rounded px-2 py-1 text-[8px] font-bold uppercase tracking-[0.09em] transition-colors ${mapMode === item ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>{item === "blackrep" ? "Black Reps" : item === "primary" ? "Primary" : item}</button>)}</div></div><div className="pointer-events-auto relative mt-2 max-w-sm"><Search className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground" size={12} /><input value={mapSearch} onChange={(event) => setMapSearch(event.target.value)} placeholder={mapMode === "primary" ? "Search a participating state" : "Search a state or person"} className="h-7 w-full rounded border border-border bg-background/85 pl-7 pr-2 text-[10px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/60" aria-label="Search election map" />{mapSearchMatches.length > 0 && <div className="absolute inset-x-0 top-8 overflow-hidden rounded border border-border bg-card shadow-lg">{mapSearchMatches.map((match) => <button key={match.stateCode} type="button" onClick={() => { setSelectedState(match.stateCode); setStateDetailOpen(true); setMapSearch(""); }} className="block w-full border-b border-border px-2 py-1.5 text-left text-[9px] text-foreground last:border-b-0 hover:bg-muted">{match.label}</button>)}</div>}</div><div className="mt-2 flex flex-wrap gap-x-2.5 gap-y-1">{mapMode === "blackrep" ? <span className="inline-flex items-center gap-1 text-[8px] text-muted-foreground"><i className="h-2 w-2 rounded-full bg-[var(--color-representation)]" />Black Political Representation presence</span> : mapMode === "primary" ? <><span className="inline-flex items-center gap-1 text-[8px] text-muted-foreground"><i className="h-2 w-2 rounded-full bg-[var(--color-primary-result)]" />August 18 primary / special result</span><span className="text-[8px] text-muted-foreground">Source linked · status stated</span></> : LEGEND.map(([label, color]) => <span key={label} className="inline-flex items-center gap-1 text-[8px] text-muted-foreground"><i className={`h-2 w-2 rounded-full ${color}`} />{label}</span>)}</div></div>
            <div className="absolute inset-x-3 bottom-3 z-10 rounded-md border border-border bg-card/94 px-3 py-2 shadow-sm"><div className="flex items-center gap-2"><span className="font-bold uppercase tracking-[0.09em] text-primary">{chamberInsight.title}</span><span className="text-muted-foreground">•</span><span className="truncate text-[9px] text-muted-foreground">{chamberInsight.summary}</span><Link href={mapMode === "blackrep" ? "/elections?tab=cbc" : "/elections"} className="ml-auto inline-flex shrink-0 items-center gap-1 text-[9px] font-semibold text-primary">More <ArrowUpRight size={10} /></Link></div><div className="mt-1.5 grid grid-cols-4 divide-x divide-border/70 text-center"><MapIntel value={chamberInsight.total} label={mapMode === "primary" ? "States" : mapMode === "blackrep" ? "Profiles" : mapMode === "house" ? "Districts" : "Races"} /><MapIntel value={chamberInsight.tossUps} label={mapMode === "primary" ? "Projected" : mapMode === "blackrep" ? "Records" : "Toss-up"} /><MapIntel value={chamberInsight.leans} label={mapMode === "primary" ? "Unofficial" : mapMode === "blackrep" ? "States" : "Lean"} /><MapIntel value={chamberInsight.calls} label={mapMode === "primary" ? "Ongoing" : mapMode === "blackrep" ? "Completed" : "Called"} /></div></div>
          </section>

          <aside className="homepage-atlas-panel flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background/55 p-3"><div className="flex items-center justify-between gap-2"><h2 className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">Daily Intelligence Brief</h2><div className="inline-flex rounded border border-border bg-background p-0.5 text-[8px] font-bold"><button onClick={() => setVoicePreference("andrew")} className={`rounded px-1.5 py-1 ${voicePreference === "andrew" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Andrew</button><button onClick={() => setVoicePreference("jenny")} className={`rounded px-1.5 py-1 ${voicePreference === "jenny" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Jenny</button></div></div>
            ...
            <div className="flex items-center gap-2.5 border-b border-border py-2.5"><button type="button" onClick={() => latestEpisodeHasAudio && play({ url: selectedFullEpisodeUrl, alternateUrl: voicePreference === "andrew" ? latestEpisode.jennyFullEpisodeCdnUrl : latestEpisode.fullEpisodeCdnUrl, voice: voicePreference, title: `The Daily Intelligence Brief · ${voicePreference === "andrew" ? "Andrew" : "Jenny"}`, episodeDate: latestEpisode.date })} disabled={!latestEpisodeHasAudio} className="grid h-8 w-8 place-items-center rounded-full bg-primary text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"><Play size={14} fill="currentColor" /></button><span className="text-[10px] font-semibold text-foreground">{latestEpisodeHasAudio ? `Play ${voicePreference === "andrew" ? "Andrew" : "Jenny"} episode` : `${voicePreference === "jenny" ? "Jenny mix" : "Audio"} preparation`}</span>{latestEpisode && <div className="ml-auto flex items-center gap-1">{latestEpisode.fullEpisodeCdnUrl && <a href={latestEpisode.fullEpisodeCdnUrl} download={`daily-intelligence-brief-${latestEpisode.date}-andrew.mp3`} title="Download Andrew full episode" className="inline-flex items-center gap-1 rounded border border-border px-1.5 py-1 text-[8px] font-bold text-primary hover:bg-primary/10"><Download size={10}/>Andrew</a>}{latestEpisode.jennyFullEpisodeCdnUrl && <a href={latestEpisode.jennyFullEpisodeCdnUrl} download={`daily-intelligence-brief-${latestEpisode.date}-jenny.mp3`} title="Download Jenny full episode" className="inline-flex items-center gap-1 rounded border border-border px-1.5 py-1 text-[8px] font-bold text-primary hover:bg-primary/10"><Download size={10}/>Jenny</a>}</div>}</div>
            <div className="min-h-0 flex flex-1 flex-col pt-2"><div className="mb-1 shrink-0 rounded border border-primary/25 bg-primary/[0.045] px-2 py-1"><p className="text-[8px] font-bold uppercase tracking-[0.12em] text-primary">{currentTrack?.episodeDate === latestEpisode?.date && currentTrack.segmentKey ? `Now playing · ${currentTrack.segmentRole === "greeting" ? "Opening" : currentTrack.segmentRole === "closing" ? "Closing" : "Editorial"}${currentTrack.segmentOrdinal && currentTrack.segmentTotal ? ` · ${currentTrack.segmentOrdinal}/${currentTrack.segmentTotal}` : ""}` : `Episode segments · ${voicePreference === "andrew" ? "Andrew" : "Jenny"}`}</p><p className="truncate text-[8px] text-muted-foreground">{currentTrack?.episodeDate === latestEpisode?.date && currentTrack.segmentKey ? currentTrack.title : "Opening greeting → 13 editorial segments → closing"}</p><div className="mt-1 h-px overflow-hidden bg-muted"><div className="h-full bg-primary" style={{ width: `${currentTrack?.episodeDate === latestEpisode?.date && duration > 0 ? Math.min(100, (progress / duration) * 100) : 0}%` }} /></div></div><div className="min-h-0 flex-1 divide-y divide-border/60 overflow-y-auto pr-1 [scrollbar-color:var(--primary)_transparent] [scrollbar-width:thin]">{(segments.length ? segments : [{ label: "Opening greeting" }, { label: "Editorial analysis" }, { label: "Closing" }]).map((segment, index) => { const url = voicePreference === "andrew" ? segment.audioPath : segment.jennyAudioPath; const alternateUrl = voicePreference === "andrew" ? segment.jennyAudioPath : segment.audioPath; const hasAudio = Boolean(url); const segmentRole = getDailyBriefSegmentRole(segment.key); const isActive = currentTrack?.episodeDate === latestEpisode?.date && currentTrack.segmentKey === segment.key; return <button key={segment.key ?? index} type="button" onClick={() => hasAudio && play({ url, alternateUrl, voice: voicePreference, title: segment.label, episodeDate: latestEpisode?.date ?? "", segmentKey: segment.key, segmentOrdinal: index + 1, segmentTotal: segments.length, segmentRole })} disabled={!hasAudio} className={`flex w-full gap-2 py-1.5 text-left text-[10px] disabled:cursor-not-allowed disabled:opacity-50 ${isActive ? "bg-primary/10" : ""}`}><span className="text-primary">{index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate text-foreground/80">{segment.label}</span><span className="block text-[8px] uppercase tracking-[.1em] text-muted-foreground">{segmentRole === "greeting" ? "Opening" : segmentRole === "closing" ? "Closing" : "Editorial"}</span></span><span className="text-muted-foreground">{segment.durationLabel ?? "—"}</span></button>; })}</div><p className="mt-1 text-[8px] text-muted-foreground">Both full episode voices are verified; choose Andrew or Jenny for full or segment playback.</p><Link href="/podcast" className="mt-2 inline-flex shrink-0 items-center gap-1 text-[9px] font-semibold text-primary">Open full briefing <ArrowUpRight size={10} /></Link></div>
      ...
      <Dialog open={stateDetailOpen} onOpenChange={setStateDetailOpen}>
        <DialogContent className="max-h-[82vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{stateTitle}</DialogTitle></DialogHeader><div className="space-y-2">{selectedRaces.length ? selectedRaces.map((race, index) => mapMode === "primary" ? <PrimaryResultsDetail key={race.stateCode} record={race} /> : mapMode === "blackrep" ? <BlackRepresentationDetail key={`${race.popupType}-${race.id ?? index}`} record={race} /> : <RaceDetail key={race.id ?? index} race={race} mode={mapMode} />) : <p className="rounded border border-border p-3 text-sm text-muted-foreground">No {mapMode === "primary" ? "August 18 primary or special-election result" : mapMode === "blackrep" ? "Black Representation profiles or article-backed election records" : mapMode === "house" ? "House races" : mapMode === "governor" ? "Governor race" : "Senate race"} are currently tracked for this state.</p>}</div></DialogContent>
      </Dialog>
    </div>
  );
}
