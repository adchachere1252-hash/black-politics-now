    ...
    <div className="hidden h-[calc(100dvh-64px)] overflow-hidden bg-background p-2 lg:block">
      <main className="mx-auto flex h-full max-w-[1640px] min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card p-2 shadow-[0_18px_70px_rgba(17,24,39,0.12)] dark:shadow-black/35">
        ...
        <section className="grid min-h-0 flex-1 grid-cols-[0.92fr_1.68fr_0.96fr] grid-rows-[minmax(0,1fr)] gap-2">
          ...
          <section className="grid h-[clamp(116px,16vh,146px)] shrink-0 grid-cols-2 gap-2">
            <Link href="/world" className="relative overflow-hidden rounded-lg border border-cyan-400/30 bg-[radial-gradient(circle_at_79%_32%,rgba(84,198,255,0.3),transparent_43%),linear-gradient(135deg,#06162d,#0a2647_55%,#0d1727)] p-2.5 shadow-[inset_0_0_35px_rgba(48,165,255,0.13)] transition-colors hover:border-cyan-300/80"><div className="relative z-10 max-w-[54%]"><div className="flex items-start justify-between gap-2"><p className="text-[8px] font-bold uppercase tracking-[0.15em] text-cyan-100"><Globe2 className="mr-1 inline" size={10} />World Elections</p><span className="rounded border border-cyan-200/30 bg-cyan-100/10 px-1.5 py-0.5 text-[8px] font-bold text-cyan-100">{worldBrief.total}</span></div><h3 className="mt-1 text-sm font-bold leading-tight text-white">Global elections at a glance.</h3><p className="mt-1 text-[8px] uppercase tracking-wide text-cyan-100/75">{worldBrief.live ? `${worldBrief.live} voting now` : "Next ballot"}</p>{worldBrief.upcoming && <div className="mt-1.5 border-l-2 border-cyan-200/80 pl-1.5"><p className="truncate text-[9px] font-semibold text-white">{worldBrief.upcoming.country}</p><p className="text-[8px] text-cyan-100/75">{shortDate(worldBrief.upcoming.electionDate)} · {worldBrief.upcoming.electionType}</p></div>}</div><div aria-hidden className="absolute bottom-1 right-1 h-[108px] w-[108px] drop-shadow-[0_0_16px_rgba(89,220,255,0.8)]"><MiniRepositoryGlobe theme={theme} vibrant /></div></Link>

            <Link href="/atlas" className="relative overflow-hidden rounded-lg border border-border bg-card p-2.5 transition-colors hover:bg-muted/50"><img src="/manus-storage/selma-marchers-homepage_4dbcaa12.jpg" alt="Civil rights marchers crossing the Edmund Pettus Bridge in Selma" className="absolute inset-y-0 right-0 h-full w-[41%] object-cover opacity-18 grayscale dark:opacity-38" /><div className="absolute inset-0 bg-gradient-to-r from-card via-card/95 to-card/20 dark:via-card/88 dark:to-transparent" /><div className="relative z-10 max-w-[67%]"><div className="flex items-start justify-between gap-2"><p className="text-[8px] font-bold uppercase tracking-[0.15em] text-primary"><Landmark className="mr-1 inline" size={10} />Voting Rights Act</p><ArrowUpRight className="mt-0.5 text-primary" size={10} /></div><h3 className="mt-1 text-sm font-bold leading-tight text-foreground">History behind today’s map.</h3><p className="mt-1 text-[8px] leading-snug text-muted-foreground">VRA, representation, apportionment, and active redistricting in one civic record.</p><div className="mt-2 grid grid-cols-3 divide-x divide-border rounded border border-border bg-background/80 text-center"><AtlasStat value="1965" label="VRA" /><AtlasStat value={atlasBrief.tracked} label="States" /><AtlasStat value={atlasBrief.litigated} label="Cases" /></div></div></Link>
          </section>
          </div>
