...
import { ArchiveRestore, ArrowUpRight, Clock3, Headphones, MapPin, Newspaper, Search, ShieldCheck } from "lucide-react";
...
export default function ArchivePage() {
  ...
  const { data: certifiedArchives = [] } = trpc.certifiedResultsArchive.list.useQuery();
  const [selectedArchiveKey, setSelectedArchiveKey] = useState<string | null>(() => typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("results"));
  const { data: selectedArchive, isLoading: archiveLoading } = trpc.certifiedResultsArchive.detail.useQuery({ archiveKey: selectedArchiveKey ?? "unavailable" }, { enabled: Boolean(selectedArchiveKey) });
  ...
  return (
    <div className="container py-8">
      ...
      {tab === "elections" && (
        <section className="space-y-4"><div className="rounded-xl border border-primary/25 bg-primary/[0.045] p-5"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-primary">Certified record only</p><h2 className="mt-2 flex items-center gap-2 text-lg font-bold"><ShieldCheck size={18} className="text-primary" /> Post-certification results archive</h2><p className="mt-2 max-w-3xl text-sm text-muted-foreground">Only snapshots created after an election authority’s certification appear here. Live returns, projected calls, and preliminary results remain outside this archive.</p></div>{(certifiedArchives as any[]).length ? <div className="grid gap-3 lg:grid-cols-[.9fr_1.1fr]"><div className="space-y-2">{(certifiedArchives as any[]).map((archive: any) => <button key={archive.archiveKey} onClick={() => setSelectedArchiveKey(archive.archiveKey)} className={`w-full rounded-lg border p-4 text-left transition-colors ${selectedArchiveKey === archive.archiveKey ? "border-primary/50 bg-primary/[0.07]" : "border-border bg-card hover:border-primary/30"}`}><p className="flex items-center gap-2 text-sm font-bold"><ArchiveRestore size={15} className="text-primary" />{archive.title}</p><p className="mt-1 text-xs text-muted-foreground">{archive.entryCount} certified entries · {new Date(archive.certifiedAt).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}</p><p className="mt-2 text-xs text-primary">Authority: {archive.certificationAuthority}</p></button>)}</div><div className="rounded-xl border border-border bg-card p-5">{archiveLoading ? <div className="h-48 animate-pulse rounded bg-muted" /> : selectedArchive ? <><p className="text-[10px] font-bold uppercase tracking-[.15em] text-primary">Immutable snapshot</p><h3 className="mt-1 text-lg font-bold">{selectedArchive.archive.title}</h3><p className="mt-2 text-sm text-muted-foreground">{selectedArchive.archive.certificationStatement}</p><a className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline" href={selectedArchive.archive.certificationSourceUrl} target="_blank" rel="noreferrer">Open certification authority source <ArrowUpRight size={12}/></a><div className="mt-4 space-y-2">{selectedArchive.entries.map((entry: any) => <article key={entry.id} className="rounded-lg border border-border/70 bg-background/45 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold">{entry.resultLabel}</p><span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary">{entry.chamber}</span></div><p className="mt-1 text-xs text-muted-foreground">{entry.winnerOrResult}{entry.partyOrSide ? ` · ${entry.partyOrSide}` : ""}</p><a className="mt-2 inline-flex items-center gap-1 text-[11px] text-primary hover:underline" href={entry.sourceUrl} target="_blank" rel="noreferrer">Result evidence <ArrowUpRight size={11}/></a></article>)}</div><p className="mt-4 text-[11px] text-muted-foreground">Snapshot digest: {selectedArchive.archive.snapshotDigest}</p></> : <p className="text-sm text-muted-foreground">Choose a certified archive to inspect its preserved ledger.</p>}</div></div> : <div className="rounded-xl border border-dashed border-border px-5 py-10 text-center"><ArchiveRestore className="mx-auto text-primary" size={22}/><p className="mt-3 text-sm font-semibold">No certified result archive yet</p><p className="mx-auto mt-1 max-w-lg text-xs text-muted-foreground">The archive will remain empty until an administrator records an election authority, a certification source, a certification date, and a result ledger made exclusively of Certified records.</p></div>}</section>
      )}
    </div>
  );
}
