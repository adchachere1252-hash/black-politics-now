...
export default function Podcast() {
  ...
  const filteredEpisodes = useMemo(() => {
    ...
    return all.filter(ep =>
      (!searchQuery || ep.date.includes(q) || ep.day?.toLowerCase().includes(q) || ep.segments.some((s: any) => `${s.label || ""} ${s.script || ""} ${JSON.stringify(s.sourceLinks || [])}`.toLowerCase().includes(q))) &&
